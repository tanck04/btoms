...
//        }
     ...
     */
    public List<Applicant> getPendingApplicantsByProject(Project project) {
        ...
        // First, find all applications for this project with PENDING status
        for (Application application : ApplicationRepository.APPLICATIONS.values()) {
            if (projectID.equals(application.getProjectID()) &&
                ...
                // Get the applicant associated with this application
                Applicant applicant = ApplicantRepository.APPLICANTS.get(application.getApplicant().getNRIC());

                if (applicant != null && !pendingApplicants.contains(applicant)) {
